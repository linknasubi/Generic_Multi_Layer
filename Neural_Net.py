...
hidden_layers = 2 # Refere-se a quantidade de camadas desejáveis
layer_counter = 0
nodes = 4
...
nodes_arrays = np.dot(feature_set,weights_layers_input)+bias
...
error_cost = (1/len(nodes_layers_arrays[-1][0]))*((output_layer_sig - labels)**2)
print(error_cost)
...
for i in range(len(nodes_layers_arrays)):
    ...
    dcost_da = (2/len(nodes_layers_arrays[layer_counter][0])) * (output_layer_sig - labels)
    da_dz = sigmoid_derv(output_layer_dot)
    dz_dw = nodes_layers_arrays[layer_counter]
    
    dcost_w = np.dot(dz_dw.T, dcost_da*da_dz)
    
    derv_layers_arrays.append(dcost_w)
